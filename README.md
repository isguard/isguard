...
**Tiny, dependency-free TypeScript type guards — headlined by `isRecord`.**

70+ `value is T` predicates, assertions, and combinators that narrow `unknown`
without a single cast. Zero dependencies, fully tree-shakable, ships **ESM +
CJS + types**. Runs anywhere ES2022 does — **browser, Node, Bun, Deno**.

[![npm](https://img.shields.io/npm/v/isguard)](https://www.npmjs.com/package/isguard)
[![types](https://img.shields.io/npm/types/isguard)](https://www.npmjs.com/package/isguard)
[![bundle](https://img.shields.io/bundlephobia/minzip/isguard)](https://bundlephobia.com/package/isguard)
[![license](https://img.shields.io/npm/l/isguard)](./LICENSE)
...
```ts
import { isRecord } from 'isguard';
```

**Deno** (no install needed):

```ts
import { isRecord } from 'npm:isguard';
```
...
```ts
import { isRecord, isString, isArrayOf, isOneOf, assertDefined } from 'isguard';
...
```ts
import { isJsonValue } from 'isguard';
...
```ts
import { union, optional, isRecordOf, isString, isNumber } from 'isguard';
...
```ts
import { assertNever } from 'isguard';
...
```ts
import type { Guard } from 'isguard';
